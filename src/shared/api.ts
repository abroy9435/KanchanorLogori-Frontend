...
// --------------------
// DISCOVER APIs — with SW bypass
// --------------------

// Search users
export const discoverSearch = (body: { search_parameter: string }) =>
  api.post("/discover/search", body).then((r) => r.data);

// Get profile by UID
export const discoverGetUserByUid = (body: { user_uid: string }) =>
  api.post("/discover/byUid", body).then((r) => r.data);

// Discover feed posts with pagination
export const discoverGetFeedPosts = (page: number) =>
  getFresh(`/posts/feed/${page}`).then((r) => r.data);
