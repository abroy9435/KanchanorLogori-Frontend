...
import {
  ...
  getProgrammes,
  getDepartments,
} from "../../shared/services/optionService";
...
export default function Register() {
  ...
  const [form, setForm] = useState({
    ...
    // Page 4
    looking_for: null as number | null,
  ...
  const YEARS = useMemo(
    () => Array.from({ length: 70 }, (_, i) => now.getFullYear() - i),
    [now]
  );
  ...
  useEffect(() => {
    getSchools()
      ...
      .catch(console.error);
    getProgrammes()
      .then((res: any[]) => setProgrammes(res.map((p: any) => ({ id: p.id, name: p.programme_name }))))
      .catch(console.error);
    getDepartments()
      .then((res: any[]) => setDepartments(res.map((d: any) => ({ id: d.id, name: d.department_name }))))
      .catch(console.error);
  }, []);
    ...
    <div className="min-h-screen bg-[#0D0002] text-white">
      ...
      <div className="px-4 pt-6">
        <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
          <div
            className="h-full bg-[#FF5069] transition-all"
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
        ...
        {step === 0 && (
          <div className="space-y-6">
            <div>
              ...
              <div className="px-4 py-3 bg-black/40 rounded-xl">{displayName || "—"}</div>
            </div>
            <div>
              ...
              {(["male", "female"] as const).map((g) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={active ? "text-[#FF5069]" : ""}>
            ...
            <div className="mt-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                <Wheel
                  ...
                  options={DAYS}
                  value={dobDay}
                  onChange={(v) => setDobDay(Number(v))}
                />
                <Wheel
                  ariaLabel="Month"
                  options={MONTHS}
                  value={dobMonth ? MONTHS[dobMonth - 1] : null}
                  onChange={(v) => setDobMonth(MONTHS.indexOf(String(v)) + 1)}
                />
                <Wheel
                  ariaLabel="Year"
                  options={YEARS}
                  value={dobYear}
                  onChange={(v) => setDobYear(Number(v))}
                />
              </div>
            </div>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full border transition ${
                      selected
                        ? "bg-[#FF5069] border-[#FF5069] text-white"
                        : `bg-transparent border-white/20 ${full ? "opacity-40" : ""}`
                    }`}
                    disabled={full}
                  >
                    {i.name}
            ...
            {["Introvert", "Extrovert", "Ambivert", "Omnivert"].map((p) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl ${
                  form.personality === p ? "bg-[#FF5069] text-white" : "bg-white/5"
                }`}
              >
                {p}
              </button>
          ...
          <div className="space-y-3 mt-2">
            {LOOKING_FOR_OPTIONS.map((opt) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl ${
                  form.looking_for === opt.id ? "bg-[#FF5069] text-white" : "bg-white/5"
                }`}
              >
                {opt.label}
              </button>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4 bg-white/5 rounded-2xl p-4">
              ...
              <div className="flex-1">
                ...
                <label className="inline-block mt-1 px-3 py-2 rounded-xl bg-[#FF5069] text-white cursor-pointer">
                  Upload & Crop
                  <input
                    type="file"
                    accept="image/*"
            ...
            <div>
              ...
              <textarea
                className="w-full h-40 bg-white/5 rounded-2xl px-4 py-3"
      ...
      {/* footer nav */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#0D0002] px-5 py-4">
        <div className="flex justify-between gap-3">
          ...
          {step < totalSteps - 1 ? (
            <button
              type="button"
              className="px-6 py-3 rounded-2xl bg-[#FF5069] text-white"
              onClick={() => setStep((s) => clamp(s + 1, 0, totalSteps - 1))}
