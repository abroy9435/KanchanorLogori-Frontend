...
//         className="w-[1rem] h-[1rem] border-2 border-white border-t-transparent rounded-full"
//       />
//     </div>
//   );
// }
...
//   // -----------------------------
//   // FIX 1: Safe provider detection (no infinite loops)
//   // -----------------------------
//   // useEffect(() => {
...
//     // const alreadyDone = localStorage.getItem("passwordAdded");

//   //   if (hasGoogle && !hasPassword && !alreadyDone) {
//   //     setNeedsPassword(true);
//   //   }
//   // }, [user, loading]);
//   useEffect(() => {
//     setNeedsPassword(false); // completely disable popup
//   }, []);
  
//   // -----------------------------
//   // FIX 2: Attach Password Flow
//   // -----------------------------
...
//     if (!auth.currentUser) {
//       alert("Auth error. Please re-login.");
//       return;
//     }
...
//     } catch (err: unknown) {
//       const e = err as any;
//       alert("Failed to attach password: " + (e?.message ?? "Unknown error"));
//     } finally {
...
//         if (code === 404) setStatus("register");
//         else if (code === 401) setStatus("login");
//         else {
//           console.error(err);
//           setStatus("error");
//         }
...
//   // -----------------------------
//   // FIX 3: Navigation (no rapid replaceState loops)
//   // -----------------------------
//   useEffect(() => {
...
//   if (status === "loading") {
//     return (
//       <div className="min-w-screen fixed inset-0">
//         <div className="text-white p-8 flex gap-2 justify-center items-center w-screen h-screen italic">
...
/* ---------------------------------------------------------
   ADD PASSWORD OVERLAY (kept but unused)
--------------------------------------------------------- */
function AddPasswordOverlay({
  visible,
  onSubmit,
  ...
  return (
    <div className="fixed inset-[0rem] backdrop-blur-[0.5rem] flex items-center justify-center z-[9999]">
      <div className="bg-[#1F0004] rounded-[1rem] w-[85%] max-w-[22rem] p-[1rem] text-center shadow-xl animate-fadeIn">
        <div className="text-white text-[1.3rem] mb-[1rem]">Add a Password</div>
        <p className="text-gray-300 text-sm mb-3">
          You can add a password to your account for easy login via Email and
          Password (Recommended for IOS Users)
        </p>
        ...
        <input
          ...
          placeholder="Create new password"
          disabled={submitting}
          className="w-[17rem] px-[1.5rem] py-[1.3rem] text-[1rem] rounded-[2rem] bg-[#0D0002] border-transparent text-white mb-[0.8rem]"
          onChange={(e) => setPassword(e.target.value)}
        />
        ...
        <button
          onClick={onSubmit}
          disabled={submitting}
          className="w-[13rem] bg-[#FF5069] text-white px-[1.5rem] py-[0.8rem] rounded-full shadow-md my-[0.5rem]"
        >
          {submitting ? <Spinner /> : "Save Password"}
        </button>
        ...
        <button
          onClick={onCancel}
          className="w-[13rem] bg-[#1A1A1A] text-white px-[1.5rem] py-[0.8rem] rounded-full shadow-md my-[0.5rem]"
        >
          Not now
        </button>
      </div>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (code === 401) {
          console.warn("[GateMobile] 401 → refreshing token…");
          ...
          try {
            ...
            console.error("[GateMobile] Token refresh failed → redirect to login");
