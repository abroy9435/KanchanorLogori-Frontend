...
export default function FeedMobile() {
  ...
  // 🔥 CHANGE START
  // New refs/state for incremental fetch + dedupe
  const seenUIDsRef = useRef<Set<string>>(new Set()); // keep track of UIDs already appended
  const fetchInProgressRef = useRef(false); // avoid concurrent fetches
  const emptyFetchesRef = useRef(0); // consecutive empty filtered fetch attempts
  const [noMoreProfiles, setNoMoreProfiles] = useState(false); // true when backend likely exhausted
  // tuning:
  const MAX_EMPTY_FETCHES = 3; // if we get 3 consecutive responses with no new UIDs, assume no more
  const FETCH_THRESHOLD = 3; // when currentIndex reaches profiles.length - FETCH_THRESHOLD, prefetch
  // 🔥 CHANGE END

  // 🔥 CHANGE START
  // Central fetcher: calls getRandomProfiles() and appends only unseen UIDs.
  // It will try a few times (to avoid returning only duplicates) and then stop.
  const fetchMoreProfiles = useCallback(async () => {
    if (fetchInProgressRef.current || noMoreProfiles) return;
    fetchInProgressRef.current = true;

    try {
      // try up to MAX_EMPTY_FETCHES times to get at least one new profile
      let attempts = 0;
      let foundAny = false;

      while (attempts < MAX_EMPTY_FETCHES) {
        attempts++;
        try {
          const data: UserProfileOnReceive[] = await getRandomProfiles();
          if (!Array.isArray(data) || data.length === 0) {
            // backend returned nothing — count as empty
            emptyFetchesRef.current += 1;
            continue;
          }

          // map -> UI and filter out seen UIDs
          const mapped = data.map(mapUserProfile);
          const unique = mapped.filter((m) => {
            if (!m?.uid) return false;
            return !seenUIDsRef.current.has(String(m.uid));
          });
          ...
          if (unique.length > 0) {
            ...
            setProfiles((prev) => {
              ...
              // register uids as seen
              unique.forEach((u) => seenUIDsRef.current.add(String(u.uid)));
              return next;
            });

            foundAny = true;
            emptyFetchesRef.current = 0;
            break; // we got some, stop trying more
          } else {
            // filtered out all as duplicates
            emptyFetchesRef.current += 1;
            // continue to attempt again (maybe backend will return other results next call)
          }
        } catch (err) {
          console.error("fetchMoreProfiles: getRandomProfiles failed:", err);
          emptyFetchesRef.current += 1;
        }
      }

      if (!foundAny && emptyFetchesRef.current >= MAX_EMPTY_FETCHES) {
        // assume backend exhausted or highly repetitious
  ...
  // 🔥 CHANGE START
  // initial loader uses the new fetcher so seenUIDsRef is maintained correctly
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
  ...
  // 🔥 CHANGE START
  // prefetch next batch when we are near the end
  useEffect(() => {
    // if we're close to end, try to fetch more
    if (!noMoreProfiles && currentIndex >= Math.max(0, profiles.length - FETCH_THRESHOLD)) {
      // schedule microtask to avoid interfering with immediate UI transitions
      void (async () => {
        await fetchMoreProfiles();
      })();
    }
  }, [currentIndex, profiles.length, fetchMoreProfiles, noMoreProfiles]);
  // 🔥 CHANGE END
    ...
    try {
      ...
      setTimeout(() => {
        if (currentIndex < profiles.length - 1) {
          ...
          // allow move to the sentinel "no more" card by incrementing past end
          setDirection(1);
          setCurrentIndex((i) => i + 1);
        } else {
          // We are at last known profile, try to fetch more immediately
  ...
  if (currentIndex >= profiles.length) {
    // When currentIndex is past the last appended profile, show the existing "No more profiles" card.
    return (
